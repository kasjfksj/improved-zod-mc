...
class ModifiedMueller(Distribution):
    def __init__(self, A, a, b, c, XX, YY):
        ...
        # Precompute marginal samples for KDE
        self._init_marginals()
        
    def _init_marginals(self, n_samples=100):
        """Initialize marginal samples using grid sampling"""
        device = self.A.device
        x_range = torch.linspace(-1, 9, 200, device=device)
        y_range = torch.linspace(-8, 2, 200, device=device)
        xx, yy = torch.meshgrid(x_range, y_range, indexing='ij')
        grid_points = torch.stack([xx.flatten(), yy.flatten()], dim=1)
        
        log_probs = self._log_prob(grid_points).squeeze()
        probs = torch.exp(log_probs - log_probs.max())
        
        idx = torch.multinomial(probs, n_samples, replacement=True)
        samples = grid_points[idx]
        
        self.marginal_samples_x = samples[:, 0]
        self.marginal_samples_y = samples[:, 1]
        
    def log_prob_single_dim(self, x_value, dim_idx):
        """Marginal log prob using KDE"""
        marginal_samples = self.marginal_samples_x if dim_idx == 0 else self.marginal_samples_y
        bandwidth = 0.1
        
        distances = (x_value.unsqueeze(1) - marginal_samples.unsqueeze(0)) ** 2
        kernel_vals = torch.exp(-distances / (2 * bandwidth ** 2))
        density = kernel_vals.mean(dim=1) / (bandwidth * torch.sqrt(2 * torch.tensor(pi, device=x_value.device)))
        
        return torch.log(density + 1e-10)
    
    def log_prob_all_dims_batched(self, x):
        """Vectorized marginal log probs for all dimensions"""
        B = x.shape[0]
        log_probs = torch.zeros(B, 2, device=x.device)
        log_probs[:, 0] = self.log_prob_single_dim(x[:, 0], 0)
        log_probs[:, 1] = self.log_prob_single_dim(x[:, 1], 1)
        return log_probs
    ...
    def log_prob_single_dim(self, x_value, dim_idx):
        """
        Evaluate marginal log p(x_dim) using kernel density estimation.
        
        Args:
            x_value: [num_samples] - values for this dimension
            dim_idx: int - which dimension (0 for x, 1 for y)
            
        Returns:
            log_prob: [num_samples]
        """
        if dim_idx == 0:
            marginal_samples = self.marginal_samples_x
        else:
            marginal_samples = self.marginal_samples_y
        
        # Kernel density estimation with Gaussian kernel
        bandwidth = 0.1
        
        # Compute distance to all marginal samples
        # x_value: [B], marginal_samples: [N]
        # distances: [B, N]
        distances = (x_value.unsqueeze(1) - marginal_samples.unsqueeze(0)) ** 2
        kernel_vals = torch.exp(-distances / (2 * bandwidth ** 2))
        
        # Average over all samples and normalize
        density = kernel_vals.mean(dim=1) / (bandwidth * torch.sqrt(2 * torch.tensor(pi, device=x_value.device)))
...
class MultivariateGaussian(Distribution):
    ...
    def log_prob_single_dim(self, x_value, dim_idx):
        ...
        log_prob = -0.5 * torch.log(2 * torch.tensor(pi, device=x_value.device)) \
                   - torch.log(std_i) \
    ...
    def log_prob_all_dims_batched(self, x):
        ...
        Returns:
            log_probs: [B, D]
        """
        # Broadcast operations across all dimensions
        log_probs = -0.5 * torch.log(2 * torch.tensor(pi, device=x.device)) \
                    - torch.log(self.marginal_stds).unsqueeze(0) \
                    - 0.5 * ((x - self.marginal_means.unsqueeze(0)) / self.marginal_stds.unsqueeze(0)) ** 2
        return log_probs
