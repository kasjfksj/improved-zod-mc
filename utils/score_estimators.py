...
class PartialZODMC_ScoreEstimator(ScoreEstimator):
    ...
    def __init__(self, dist, sde, device, block_size=1, def_num_batches=1, def_num_rej_samples=10000, max_iters_opt=50):
        super().__init__(dist, sde, device, def_num_batches, def_num_rej_samples)
        self.block_size = block_size
        self.dim_blocks = [list(range(i, min(i + block_size, dist.dim))) 
                           for i in range(0, dist.dim, block_size)]
    ...
    def score_estimator(self, x, tt, num_batches=None, num_samples=None):
        ...
        assert num_batches > 0 and num_samples > 0

        mean_estimate = 0
        num_good_samples = torch.zeros((x.shape[0], 1), device=self.device)

        for _ in range(num_batches):
            samples_from_p0t, acc_idx = rejection_sampler.get_samples_partial(x/scaling, variance_conv,
                                                                            self.dist,
                                                                            num_samples,
                                                                            self.device)
            # acc_idx: [n, num_samples, d] — average across samples and dims
        ...
        score_full = (scaling * mean_estimate - x) / (1 - scaling**2)
        return score_full   # [n, d]
...
def get_score_function(config, dist : Distribution, sde, device):
    ...
    if config.score_method == 'p0t' and config.p0t_method == 'rejection' and config.use_partial_zodmc:
        return PartialZODMC_ScoreEstimator(dist, sde, device,
